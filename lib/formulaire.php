...
class Formulaire{
	...
	public function creerInputRadio($unnom, $unevaleur,$unLabel, $checked){
			if($checked == 1){
					$composant="<input type ='radio' name='" . $unnom . "' value ='". $unevaleur ."' checked=checked>".$unLabel;
			} else {
	...
	public function creerInputTexteRO($unNom, $unId, $uneValue , $required , $placeholder , $pattern){
		$composant = "<input type = 'text' readonly name = '" . $unNom . "' id = '" . $unId . "' ";
		if (!empty($uneValue)){
			$composant .= "value = '" . $uneValue . "' ";
		}
		if (!empty($placeholder)){
